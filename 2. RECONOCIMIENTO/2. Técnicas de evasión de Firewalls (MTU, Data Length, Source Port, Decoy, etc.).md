...
#### Escaneo Default
```bash
nmap -p- —-open -sS —-min-rate 5000 -vvv -n -Pn <target>
```
#### Capturar escaneo
```bash
tcpdump -i ens33 -w Captura.cap -v
```
#### Analizar Captura con Wireshark
```bash
wireshark -r Captura.cap &> /dev/null & disown
```
#### Analizar Captura con Tshark
```bash
tshark -r Captura.cap -Y “<filtro(opcional)>” <formato[-Tjson]> 2>/dev/null
```
