...
### Comando
```bash
-- Ver archivos que se están transmitiendo y están dentro del servicio
smbclient -L  172.17.0.1 -N

-- Otra opción para ver los archivos, carpetas y permisos que tienen
smbmap -H 172.17.0.1

--Conectarse al servicio
smbclient //172.17.0.1/myshare -N
```
### Dentro de SMB
```bash
-- Para subir un archivo, debemos estar en la máquina real en el directorio del archivo que queremos subir
PUT <file>

-- Descargar un archivo
GET <file
```
##### Utilidad para llevarnos la data del SMB a nuestra máquina local
```bash
-- Instalar CIFS
sudo apt install cifs-utils

-- Hacer un mount para llevarnos la carpeta con todas las sub-carpetas y archivos (esto es un espejo de los archivos, los cambios se verán reflejados)

sudo mount -t cifs //172.17.0.1/myshare ./mounted -o username=null,password=null,domain=,rw
